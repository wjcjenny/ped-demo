...
this["Reuters"]["Template"]["SortingSquareLegend"] = function(obj) {
obj || (obj = {});
...
self.colorDomain.forEach(function(d,i){;
__p += '\n\t\t    <div class =" col-ms-4 col-md-4 col-lg-2">\n\t\t    \t<div class="sortingSquares-legend-circle" style="background-color:' +
((__t = (self.colorRange[i])) == null ? '' : __t) +
';"></div>\n\t\t    \t<p class = "sortingSquares-legend-text">' +
((__t = (d)) == null ? '' : __t) +
'</p>\n\t\t    </div>\n\t\t    \n\t    ';
});
__p += '   \n\t</div>\n';
};
__p += '\n<div class="button-holder">\n\t<h3 class="sortingSquares-button-label">SORT BY:</h3>\n\t';
self.buttonColumns.forEach(function(d,i){;
__p += '\n\t\t<h3 dataid="' +
((__t = (d)) == null ? '' : __t) +
'" class="sortingSquares-button ';
if(d == self.initialSort){;
__p += 'selected';
};
__p += '">' +
((__t = (self.buttonText[i])) == null ? '' : __t) +
'</h3>\n\t';
});
__p += '\n</div>\n<div class="chart" id="' +
((__t = (self.targetDiv)) == null ? '' : __t) +
'Chart"></div>';
...
this["Reuters"]["Template"]["SortingSquaresTooltip"] = function(obj) {
obj || (obj = {});
var __t, __p = '', __e = _.escape, __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
with (obj) {
...
((__t = (d.name)) == null ? '' : __t) +
'</h3>\n<h2 class="">' +
((__t = (d.sentence)) == null ? '' : __t) +
' ';
if (d.sentence != 'life'){;
__p += ' years ';
};
__p += '</h2>\n<p class="">' +
((__t = (d.terrorism)) == null ? '' : __t) +
'</h2>';
...
function print() { __p += __j.call(arguments, '') }
with (obj) {
__p += '<div class=\'legendContainer\'>\n\t<div class="legend-ital">点击显示／隐藏</div>\n\t<div class=\'dateTip\'> \n\t\t';
...
Reuters.Graphics.SortingSquares = Backbone.View.extend({
	...
	render: function() {
		...
		self.svg = d3.select(self.chartId).append("svg")
		    ...
		    .attr("height", self.height + self.margin.top + self.margin.top)
			.append("g")
		...
		$('.sortingSquares').tipsy({opacity:.9, gravity:'sw', html:true});				
		...
		$(".sortingSquares-button").on("click", function(){
...
$(document).ready(function(){
    ...
    $('body').on('click', 'ol.story-list li, ol.related-list li.internal, ol.dropdown-menu.nav li', function(event){
   ...
   function route(id){
		...
		var pos = $("#"+id).offset();
		$('body, html').animate({ scrollTop: pos.top - offsetTop });
...
//			dataURL: 'http://sfb-proxy-prod-1197393060.us-east-1.elb.amazonaws.com/json/mw-disney-earns',
			dataURL:"data/building-year-count.csv",
			height:300, //if < 10 - ratio , if over 10 - hard height.  undefined - square
			columnNames:{'xicheng':"西城",'dongcheng':"东城",'haidian':"海淀"}, // undefined uses sheet headers, object will map, array matches columnNamesDisplay
			colors: [purple4,rose4,rose2],  //array or mapped object
...
//			navSpacer:true,
		...
		Reuters.Graphics.sharePrice = new Reuters.Graphics.BarChart({
			el: "#reutersGraphic-chart2",
//			dataURL: 'http://sfb-proxy-prod-1197393060.us-east-1.elb.amazonaws.com/json/mw-disney-earns',
			dataURL:"data/average-price-count.csv",
			height:300, //if < 10 - ratio , if over 10 - hard height.  undefined - square
			columnNames:{'xicheng':"西城",'dongcheng':"东城",'haidian':"海淀"}, // undefined uses sheet headers, object will map, array matches columnNamesDisplay
			colors: [purple4,rose4,rose2],  //array or mapped object
//			dataType:'value',//value, changePreMonth, CumulativeChange, percentChange, cumulate
			YTickLabel: [["","套"]], //  \u00A0  - use that code for a space.
			// xTickLabel: [["","套"]],
//			xScaleTicks: 5,
//			yScaleTicks:5,
			// dateFormat: d3.time.format("%Y"),
...
//			},
			xTickFormat:function(d){
				...
				if (dateFormat(d) == "01/01/2001"){
					return "<4"
				}
				if (dateFormat(d) == "01/01/2002"){
					return "<7"
				}
				if (dateFormat(d) == "01/01/2003"){
					return "<10"
				}
				if (dateFormat(d) == "01/01/2004"){
					return "<13"
				}
				if (dateFormat(d) == "01/01/2005"){
					return ">13"
				}
			},
//			navSpacer:true,
		...
		Reuters.Graphics.sharePrice = new Reuters.Graphics.LineChart({
			el: "#reutersGraphic-chart3",
//			dataURL: 'http://sfb-proxy-prod-1197393060.us-east-1.elb.amazonaws.com/json/mw-disney-earns',
			dataURL:"data/trading-haidian-top-2016.csv",
			height:300, //if < 10 - ratio , if over 10 - hard height.  undefined - square
			// columnNames:{'xicheng':"西城",'dongcheng':"东城",'haidian':"海淀"}, // undefined uses sheet headers, object will map, array matches columnNamesDisplay
			margin: {top: 20, right: 0, bottom: 60, left: 30},
			colors: [gray2],  //array or mapped object
			dateFormat: d3.time.format("%Y.%m"),
			hasLegend: true,
			showTip:false,
			yScaleVals: [0,20,40,60,80,100],
		});
		
		Reuters.Graphics.sharePrice = new Reuters.Graphics.LineChart({
			el: "#reutersGraphic-chart4",
//			dataURL: 'http://sfb-proxy-prod-1197393060.us-east-1.elb.amazonaws.com/json/mw-disney-earns',
			dataURL:"data/trading-xichen-top-2016.csv",
			height:300, //if < 10 - ratio , if over 10 - hard height.  undefined - square
			// columnNames:{'荣丰2008':"荣丰2008"}, // undefined uses sheet headers, object will map, array matches columnNamesDisplay
			margin: {top: 20, right: 0, bottom: 60, left: 30},
			colors: {'荣丰2008':rose4,
					"裕中东里":gray2,
					"椿树园":gray2,
					...
					"宣武门西大街":gray2,
					"宣武门西大街":gray2,
					"宣武门西大街26号院":gray2},  //array or mapped object
			dateFormat: d3.time.format("%Y.%m"),
			hasLegend: true,
			showTip:false,
			yScaleVals: [0,20,40,60,80,100],
		...
		Reuters.Graphics.sharePrice = new Reuters.Graphics.LineChart({
			el: "#reutersGraphic-chart5",
//			dataURL: 'http://sfb-proxy-prod-1197393060.us-east-1.elb.amazonaws.com/json/mw-disney-earns',
			dataURL:"data/trading-dongchen-top-2016.csv",
			height:300, //if < 10 - ratio , if over 10 - hard height.  undefined - square
			// columnNames:{'xicheng':"西城",'dongcheng':"东城",'haidian':"海淀"}, // undefined uses sheet headers, object will map, array matches columnNamesDisplay
			margin: {top: 20, right: 0, bottom: 60, left: 30},
			colors: [gray2],  //array or mapped object
 		...
 		Reuters.Graphics.sortingGraphic = new Reuters.Graphics.SortingSquares({
			...
			buttonColumns:["type","sentencegroup","terrorism"],
			buttonText:["Ideology","Length of maximum sentence","Type of plot"],
			initialSort:"terrorism",
			colorField:"type",
			colorDomain:["Islamic State","Domestic anti-government"],
			colorRange:[red3,"#bbb", blue3, blue4, black],
			maxAcross:3,
			boxSize:25,
			legendtemplate:Reuters.Template.SortingSquareLegend,
			tooltipTemplate:Reuters.Template.SortingSquaresTooltip,
